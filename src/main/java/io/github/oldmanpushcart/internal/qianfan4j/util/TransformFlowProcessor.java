...
public class TransformFlowProcessor<T, R> implements Flow.Processor<T, R> {
    ...
    public static <T, R> Flow.Publisher<R> transformOneToOne(Flow.Publisher<T> source, Function<T, R> transformer) {
        return transform(source, t -> List.of(transformer.apply(t)));
    }
